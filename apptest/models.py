...
class Appcase(models.Model):
    ...
    create_time = models.DateTimeField(
        '创建时间', auto_now=True)    # 创建时间-自动获取当前时间
...
class Appcasestep(models.Model):
    Appcase = models.ForeignKey(
        'Appcase', on_delete=models.CASCADE)    # 关联接口id
    ...
    apptestdata = models.CharField(
        '测试数据', max_length=200, null=True)       # 测试数据
